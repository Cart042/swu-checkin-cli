...
| `.env.example` | 环境变量配置模板，包含账号、并发数、日志级别、学校官网代理自动发现和各类推送通道示例。 |
...
| `README.md` | 项目使用文档，说明安装、配置检查、数字菜单、Docker、海外服务器网络边界、代理自动发现、定时任务、推送和状态码。 |
...
| `get_info.py` | 登录与接口模块，负责 Playwright 登录、验证码识别、Token 获取/缓存、代理自动发现、学校官网连通性检查、宿舍和今日任务接口读取。 |
