...
| `.github/workflows/swu-check.yml` | GitHub Actions 签到工作流，使用缓存的 Python 依赖和 Chromium headless shell 登录；每天北京时间 21:05 定时触发，是否生效由工作流自身的启用状态决定（README 说明如何开关与排查静默停用，fork 后需要手动 Enable）；手动调试开关启用时上传短期保留的脱敏文本 artifact。 |
