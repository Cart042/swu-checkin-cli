...
| `.github/workflows/swu-check.yml` | GitHub Actions 手动运行配置，默认使用纯 HTTP 登录链路，便于测试 Actions 环境是否仍可访问学校接口。 |
...
| `get_info.py` | 登录与接口模块，负责纯 HTTP 登录、Playwright 登录、验证码识别、Token 获取/缓存、代理自动发现、学校官网连通性检查、宿舍和今日任务接口读取。 |
