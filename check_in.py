...
Account configuration is provided by :mod:`config` and the interactive menu
by :mod:`menu`.  The browser/API service is imported only when an actual
check-in or an explicit network diagnostic is requested.
"""
...
def run_network_check(timeout=5):
    ...
    try:
        from school_api import check_school_connectivity
...
def main(argv=None) -> int:
    ...
    try:
        ...
        from get_info import setup_logging
