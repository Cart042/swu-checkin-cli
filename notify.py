...
def _call_with_session(session, operation, channel_name):
...
def _send_json_channel(session, url, payload, success_key, channel_name):
...
def send_dingtalk(token, secret, title, content, session=None):
    def operation(client):
        ...
        return _send_json_channel(client, url, payload, "errcode", "钉钉机器人")

    return _call_with_session(session, operation, "钉钉机器人")


def send_qywx(key, title, content, session=None):
    def operation(client):
        ...
        return _send_json_channel(client, url, payload, "errcode", "企业微信群机器人")

    return _call_with_session(session, operation, "企业微信群机器人")


def send_bark(key, url, title, content, session=None):
    def operation(client):
        ...
        with _suppress_http_debug_logging():
            response = client.get(request_url, timeout=REQUEST_TIMEOUT_SECONDS)
        return _http_ok(response)

    return _call_with_session(session, operation, "Bark")


def send_serverchan(key, title, content, session=None):
    def operation(client):
        url = f"https://sctapi.ftqq.com/{key}.send"
        payload = {"title": title, "desp": content}
        return _send_form_channel(client, url, payload, "Server酱")

    return _call_with_session(session, operation, "Server酱")


def send_pushdeer(key, title, content, session=None):
    def operation(client):
        url = "https://api2.pushdeer.com/message/push"
        payload = {"pushkey": key, "text": title, "desp": content}
        return _send_form_channel(client, url, payload, "PushDeer")

    return _call_with_session(session, operation, "PushDeer")


def _send_form_channel(session, url, payload, channel_name):
    with _suppress_http_debug_logging():
        response = session.post(
            url,
            data=payload,
...
def _send_telegram_chunk(session, url, chat_id, text):
...
def send_telegram(bot_token, chat_id, title, content, session=None):
    ...
    def operation(client):
        ...
        for chunk in chunks:
            if not _send_telegram_chunk(client, url, normalized_chat_id, chunk):
                return False
        return True

    return _call_with_session(session, operation, "Telegram")
...
def send_push(title, content, session=None):
    ...
    try:
        ...
        for channel_name, sender, args in channels:
            ...
            try:
                succeeded = bool(sender(*args, session=client))
            except Exception:
